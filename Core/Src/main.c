...
/**
  ...
  *        Аналогично реализации в PPKY: при BusOff выполняем корректный выход.
  */
...
int main(void)
{
  ...
  FDCAN_StartAll();
  (void)HAL_ADC_Start_DMA(&hadc1, (uint32_t *)MCU_IGN03_ADC_VAL, MCU_IGN03_NUM_ADC_CHANNEL);
  App_Init();
  /* USER CODE END 2 */
